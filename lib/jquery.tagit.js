        ...
        tagit: function() {
            ...
            return this.each(function() {
                ...
                if (e.is(".tagit")) {
                    ...
                    e.on("tagit-tag-added", function() {
                        autocomplete.removeClass("open");
                    });

                    // commented out as this causes an error in jQuery 1.7
                    // e.on("focusin", function(event) {
                    //     $(this).addClass("focused");
                    //     input.focus();
                    // });
                    e.on("focusout", function(event) {
                    ...
                    input.keydown(function(event) {
                        ...
                        if (keyCode === 13 || keyCode === 9) {
                            if (autocomplete.is(".open")) {
                                selection = $("li.selected", autocomplete);
                                if (selection.length) {
                                    e.tagit("addTag", selection.text());
                                    self.val("");
                                }
                            }else{
                                // enter the tag that the user has typed
                                if(self.val().length > 1){  // require tags to be at least 2 characters
                        ...
                        if (keyCode === 38 || keyCode === 40) {
                            if (autocomplete.is(".open")) {
                                ...
                                selection = $(elements).filter(".selected");
